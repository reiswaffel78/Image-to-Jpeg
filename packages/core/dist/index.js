...
async function nextAvailablePath(p) {
...
function targetPath(inputPath, outputDir) {
...
export async function convertFile(inputPath, opts) {
    ...
    if (ext === '.avif' && !sharp.format.avif?.input?.supported) {
        return { input: inputPath, skipped: true, reason: 'decoder-unavailable: avif' };
    }
    if ((ext === '.heic' || ext === '.heif') && !sharp.format.heif?.input?.supported) {
